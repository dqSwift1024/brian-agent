...
function loadNodeDetail(nodeId: string) {
  const node = sessionStore.dagNodes.find(n => n.id === nodeId)
  nodeDetail.value = node
    ? { id: node.id, label: node.label, role: node.status }
    : null
}
...
function drawMap() {
  ...
  for (const node of nodes.value) {
    ...
    const statusColor = node.status === 'user' ? '#007AFF' : node.status === 'assistant' ? '#34C759' : node.status === 'running' ? '#FF9500' : node.status === 'done' ? '#34C759' : node.status === 'error' ? '#FF3B30' : '#8E8E93'
  ...
  <div class="relative w-full h-full">
    ...
    <div v-if="selectedNodeId" class="absolute top-2 right-2 w-72 max-h-[calc(100%-1rem)] overflow-y-auto rounded-xl bg-white/95 dark:bg-apple-gray-800/95 border border-apple-gray-200 dark:border-apple-gray-700 shadow-lg p-4 backdrop-blur">
      <div class="flex items-center justify-between mb-3">
        <span class="text-sm font-semibold">消息详情</span>
        <button class="p-1 text-apple-gray-400 hover:text-apple-gray-600" @click="selectedNodeId = null"><X :size="14" /></button>
      </div>

      <div v-if="!nodeDetail" class="text-center py-8 text-apple-gray-400 text-sm">暂无详情</div>
      <div v-else class="space-y-3 text-sm">
        <p class="break-all">{{ nodeDetail.label || '—' }}</p>
        <p><span class="text-apple-gray-400">角色：</span>{{ nodeDetail.role === 'user' ? '用户' : '助手' }}</p>
        <p class="text-xs text-apple-gray-400 break-all">{{ nodeDetail.id }}</p>
      </div>
    </div>
