...
export const BUILTIN_PROMPTS: BuiltinPromptDef[] = [
  ...
  },
  {
    ...
    template: [
      ...
      '11. Tasks that merely produce recommendations/plans/suggestions (e.g. "推荐景点", "安排行程路线", "推荐交通方式", "估算预算") CAN be executed with reasonable assumptions and SHOULD stay in "nodes"; write the assumption into task_content when needed. Only exclude subtasks that truly cannot start without user data (booking/transaction).',
      '12. If ALL subtasks are planning/recommendation type, clarifications may be empty; but if the original request implies actual booking/execution (e.g. "帮我订机票"), you MUST ask for the missing parameters via clarifications.',
      '',
