...
export const BUILTIN_PROMPTS: BuiltinPromptDef[] = [
  ...
  },
  {
    ...
    template: [
      ...
      '',
      'Decompose the task into a DAG of subtasks if it is complex. Return ONLY valid JSON with format: {"nodes":[{"task_id":"1","task_content":"...","task_complexity":50,"task_domain":"","priority":1,"dependencies":[]}],"edges":[{"from_task_id":"1","to_task_id":"2"}]}',
      '只输出必要内容，保持准确、完整、简洁，不输出多余说明。',
    ].join('\n'),
  },
  {
  ...
  },
  {
    ...
    template: [
      ...
      '{',
      '  "understood_requirement": "<结合所有上下文推断整理出的用户真实完整需求>",',
      '  "match_score": <0-100 整数，表示理解后的需求与用户原始表达的匹配度/置信度评分>,',
