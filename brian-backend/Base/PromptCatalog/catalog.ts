...
export const BUILTIN_PROMPTS: BuiltinPromptDef[] = [
  ...
  },
  {
    ...
    template: [
      ...
      '10. IMPORTANT - user-parameter identification: if a subtask requires user-specific information that cannot be reasonably assumed (e.g. departure city, travel dates, budget, passenger count, contact info for real booking/transaction), do NOT include it in "nodes". Instead, put ONE concise clarification question for it into the "clarifications" array, so the system can ask the user for the missing parameters before proceeding.',
      '11. Tasks that merely produce recommendations/plans (e.g. "recommend attractions", "draft an itinerary") CAN be executed with reasonable assumptions and SHOULD stay in "nodes"; write the assumption into task_content when needed. Only exclude tasks that truly cannot start without user data.',
      '',
