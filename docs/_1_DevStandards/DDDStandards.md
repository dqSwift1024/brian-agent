...
**方法长度约束（≤20 行）**：单个方法（含签名与闭括号）原则上不超过 20 行；超过即须拆出私有方法或领域服务函数。纯声明式内容（如 SchemaInitializer 的 DDL 清单）允许通过**数据驱动表**收敛长度。分析工具：`npm run analyze:methods`（生成 `docs/MethodIndex/method-length-report.md` 超长方法队列）。
